...
public class SwerveModule {
    ...
    public void drive(Vector strafeVector, double rotationalMagnitude, double currentRobotDegree,
            boolean relativeVelocityControl, boolean disable, boolean climbMode) {
         ...
         * *(rotationalVector.getMagnitude()/Math.abs(rotationalVector.getMagnitude())))
         * ;
         * if (strafeVector.getMagnitude()>0.5){
         * strafeVector.setMagnitude(0.5 *
         * (strafeVector.getMagnitude()/Math.abs(strafeVector.getMagnitude())));
         * }
         * }
         * else{
        ...
        if (relativeVelocityControl == false) {
            strafeVector = new Vector(strafeVector.getMagnitude(), strafeVector.getDegrees() - currentRobotDegree,
                    true);
        }
    ...
    public double getDriveSensorPosition(boolean isRedAlliance) {
        ...
        // }
        return driveMotor.getPosition().getValueAsDouble() / 19.607;
        // 25.97290039;//* Constants.SwerveConstants.wheelRotationPerMotorRotation*(2 *
        // Math.PI* Constants.SwerveConstants.wheelRadius);//CHANGE
    }
    ...
    public double getDriveSensorVelocity(boolean isRedAlliance) {
        ...
        return driveMotor.getVelocity().getValueAsDouble() / 19.607;
        // 25.97290039);// *
        // Constants.SwerveConstants.wheelRotationPerMotorRotation)/60) * 2 * Math.PI *
        // Constants.SwerveConstants.wheelRadius);//CHANGE
    }
    ...
    public Rotation2d get2dCurrentModuleDegree(boolean velocityCheck) {
        ...
        // if (velocityCheck){
        // x = new Rotation2d((((currentModuleDegree + currentRobotDegree) % 360) *
        // Math.PI)/180);
        // }
        // else{
    ...
    public SwerveModuleState getSwerveModuleState(boolean isRedAlliance) {
        return new SwerveModuleState(getDriveSensorVelocity(isRedAlliance), get2dCurrentModuleDegree(true));// speed
                                                                                                            // meters
                                                                                                            // per
                                                                                                            // second ,
                                                                                                            // rotation
